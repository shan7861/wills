...
<?php
	$selected = mysqli_select_db($con, "epbwills2019")
	or die("Could not select examples");

	$sql = "INSERT INTO wills_2019 (member_id, willtype, tname, gender, tcounty, childnames, residual, e1name, e1relationship, e2name, e2relationship)
	VALUES
	('$_POST[memberid]','3', '$_POST[tname]', '$_POST[tGender]', '$_POST[tcounty]', '$_POST[childnames]', '$_POST[residual]', '$_POST[e1name]', '$_POST[e1relationship]', '$_POST[e2name]', '$_POST[e2relationship]')";
...
<div style="text-indent: 2em;">(a) <span style="padding-left: 20px;">As of the date of the making of this Will, I am not married.</span></div>
...
<h5>ITEM IV</h5>
<div>I give all  of my property and estate, both real and personal, of whatever kind and wherever located, including any lapsed or void legacy or devise, (but not including any property over which I may have the power of disposition or appointment) that I own or to which I shall be in any manner entitled at the time of my death (collectively referred to as my “residuary estate”), to my children equally, <b><?php echo $_POST['childnames']?></b>, per stirpes. If at the time of my death there should be no beneficiary to take the property passing under this Item, then the property for which there is no named or described beneficiary shall be distributed to <b><?php echo $_POST['residual']?></b>.
...
<div style="text-indent: 2em;">b.	<span style="padding-left: 20px;">I direct that no Executor shall be required to file or furnish any bond, surety or other security in any jurisdiction, nor shall my Executor be required to file any inventory or other reports with any court.</span></div>

<div style="text-indent: 2em;">c.	<span style="padding-left: 20px;">No Executor serving hereunder shall be required to inquire into or audit the acts or doings of any predecessor Executor or to make claim against any such predecessor or the estate of such predecessor. </span></div>

<div style="text-indent: 2em;">d.	<span style="padding-left: 20px;">Any successor Executor shall have and may exercise all of the powers, privileges, immunities and exemptions conferred upon the predecessor Executor as fully and to the same extent as if such successor had originally been named as Executor hereunder.</span></div>
